...
Both cues are the same coin jingle, shipped in `Sounds/coin.ogg` (`coin.mp3` is
the same audio, kept as a one-line fallback if a given client won't play the
Ogg). An item that is both an upgrade and high value plays it **once**, not
twice. To use your own, drop a 44.1 kHz Ogg or MP3 in `Sounds/` and point the
constant at the top of `Core/Alert.lua` at it — both a missing file and a bad sound name fail silently
